...
async function loadSchedule(sql: any, ownerId: string) {
  ...
  const [config] = configs as WeeklyScheduleConfigRow[];
  ...
  return {
    ...config,
    slots: slots as WeeklyScheduleSlotRow[]
  };
}
...
export default withAuthenticatedEndpoint('GET,POST,OPTIONS', async ({ req, res, auth }) => {
  ...
  try {
    ...
    const input = (typeof req.body === 'string' ? JSON.parse(req.body) : req.body ?? {}) as WeeklyScheduleInput;
    ...
    const normalizedSlots = slots.map((slot, index) => ({
      ...
      studentEnrollmentIds: Array.isArray(slot.studentEnrollmentIds)
        ? slot.studentEnrollmentIds
            ...
            .filter((studentEnrollmentId): studentEnrollmentId is string => Boolean(studentEnrollmentId))
        : []
    }));
    ...
    if (savedConfigId) {
      ...
      const [updatedConfig] = updatedRows as Array<{ id: string }>;
      ...
      const [createdConfig] = createdRows as Array<{ id: string }>;
    ...
    for (const slot of normalizedSlots) {
      ...
      const [insertedSlot] = insertedSlots as Array<{ id: string }>;
