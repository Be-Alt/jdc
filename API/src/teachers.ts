...
async function listTeachers(sql: any, organizationId: string) {
...
export default withMethodPermissions('GET,POST,PUT,DELETE,OPTIONS', {
  ...
  try {
    ...
    const payload = (typeof req.body === 'string' ? JSON.parse(req.body) : req.body ?? {}) as TeacherInput;
    ...
    if (req.method === 'POST') {
      ...
      const [created] = insertedRows as Array<{ id: string }>;
      const teachers = await listTeachers(sql, auth.organizationId);
    ...
    if (req.method === 'PUT') {
      ...
      const [updated] = updatedRows as Array<{ id: string }>;
    ...
    if (req.method === 'DELETE') {
      ...
      const [deleted] = deletedRows as Array<{ id: string }>;
