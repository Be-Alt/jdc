...
export default withAuthenticatedEndpoint('POST,OPTIONS', async ({ req, res, auth }) => {
  try {
    ...
    if (enrollmentId) {
      ...
      const [existingStudent] = existingRows as Array<{
      ...
      const [activeSchoolRow] = activeSchoolRows as Array<{ id: string }>;
      ...
      if (teacherIds.length > 0) {
        ...
        for (const teacher of validTeacherRows as Array<{ id: string }>) {
          await sql`
      ...
      if (accommodationIds.length > 0 && dysIds.length > 0) {
        ...
        for (const accommodation of validAccommodationRows as Array<{ id: number }>) {
          await sql`
    ...
    const [person] = personRows as Array<{ id: string }>;
    ...
    const [enrollment] = enrollmentRows as Array<{ id: string }>;
