...
async function enrollmentBelongsToOrganization(sql, organizationId, enrollmentId, programId) {
...
export default withPermissionEndpoint('GET,PUT,OPTIONS', 'teaching.manage', async ({ req, res, auth }) => {
    try {
        ...
        if (req.method === 'GET') {
            ...
            const programId = getQueryParam(req.url, 'programId');
      ...
      `;
        ...
        const validItems = assessments.filter((item) => (item.itemType === 'skill' || item.itemType === 'competence' || item.itemType === 'resource') &&
